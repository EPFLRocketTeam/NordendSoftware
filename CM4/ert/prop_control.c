...
//static void init_venting(CONTROL_INST_t * control);
//7static void init_pressurisation(CONTROL_INST_t * control);
static void init_countdown(CONTROL_INST_t * control);
static void init_ignition(CONTROL_INST_t * control);
static void init_powered(CONTROL_INST_t * control);
static void init_thrust(CONTROL_INST_t * control);
static void init_shutdown(CONTROL_INST_t * control);
static void init_apogee(CONTROL_INST_t * control);
static void init_depressurisation(CONTROL_INST_t * control);
...
static void venting(CONTROL_INST_t * control);
static void pressurisation(CONTROL_INST_t * control);
static void countdown(CONTROL_INST_t * control);
static void ignition(CONTROL_INST_t * control);
static void powered(CONTROL_INST_t * control);
static void thrust(CONTROL_INST_t * control);
static void shutdown(CONTROL_INST_t * control);
static void apogee(CONTROL_INST_t * control);
static void depressurisation(CONTROL_INST_t * control);
static void glide(CONTROL_INST_t * control);
static void abort(CONTROL_INST_t * control);
...
void control_thread(void * arg) {
	...
	for(;;) {
		...
		// 	venting(&control);
		// 	break;
        // case CS_PRESSURISATION:
		// 	presurisation(&control);
		// 	break;
		case CS_COUNTDOWN:
			countdown(&control);
			break;
...
static void control_update(CONTROL_INST_t * control) {
	...
	while(can_msgPending()) {
		...
		if(control->msg.id == DATA_ID_COMMAND){
            ...
            // if(control->msg.data == COMMAND_PRESSURISATION) {
			// 	control_pressurisation();
			// }
